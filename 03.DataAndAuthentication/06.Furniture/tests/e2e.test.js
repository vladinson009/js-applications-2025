...
describe('E2E tests', function () {
  ...
  describe('Navigation bar', () => {
    ...
    it('logged user should see correct navigation', async () => {
      ...
      await Promise.all([
        page.waitForSelector('#user', { timeout: interval }),
        page.click('#login-form >> text=Login', { timeout: interval }),
      ]);
    
      const isGuestVisible = await page.isVisible('#guest');
      const isUserVisible = await page.isVisible('#user');
    
      expect(isGuestVisible).to.be.false;
      expect(isUserVisible).to.be.true;
    });
  ...
  describe('Catalog', () => {
    it('view furniture', async () => {
      ...
      await page.waitForSelector('.table', { timeout: interval });
    ...
    it('nothing bought', async () => {
      ...
      await page.click('#login-form >> text=Login', { timeout: interval });

      const { get2 } = await handle(endpoints.owner(data._id));
      get2(data);

      await page.click('#show-orders-btn', { timeout: interval });
      await page.waitForSelector('.orders', { timeout: interval });
    ...
    it('bought fruniture', async () => {
      ...
      await page.click(`.table tbody tr input[type="checkbox"]`, { timeout: interval });
  ...
  describe('CRUD', () => {
    ...
    it('create does NOT work with empty fields', async () => {
      ...
      await page.click('text=Create Product', { timeout: interval });
      await page.waitForSelector('#create-form', { timeout: interval });

      page.click('#create-form >> text=Create', { timeout: interval });
