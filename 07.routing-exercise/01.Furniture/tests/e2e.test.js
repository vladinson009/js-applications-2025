...
describe('E2E tests', function () {
  ...
  describe('Authentication', () => {
    it('Register makes correct API call', async () => {
      ...
      await page.waitForSelector('#registerLink', { timeout: interval });
    ...
    it('Login makes correct API call', async () => {
      ...
      await page.goto(host);
      await page.waitForSelector('#loginLink', { timeout: interval });
  ...
  describe('Navigation bar', () => {
    ...
    it('User should see correct navigation after click on Logout button', async () => {
      ...
      await page.waitForSelector('#logoutBtn', { timeout: interval });
      await page.click('nav >> text=Logout', { timeout: interval });
  ...
  describe('Catalog', () => {
    it('Show catalog', async () => {
      ...
      await page.goto(host);
      await page.waitForSelector('.card', { timeout: interval });
    ...
    it('Guest does NOT see edit/delete buttons', async () => {
      ...
      const { onResponse } = get(data);
      await page.goto(host);
      await page.waitForSelector('.container', { timeout: interval });

      await Promise.all([
        onResponse(),
        page.click(`.card-body:has-text("${data.description}") >> text=Details`, {
          timeout: interval,
        }),
      ]);
  ...
  describe('CRUD', () => {
    it('Create does NOT work with empty fields', async () => {
      ...
      await page.click('text=Create Furniture', { timeout: interval });
      await page.waitForSelector('form', { timeout: interval });
    ...
    it('Edit should populate form with correct data', async () => {
      ...
      await page.waitForSelector('.btn-info', { timeout: interval });
    ...
    it('Edit does NOT work with empty fields', async () => {
      ...
      await page.click('nav >> text=Dashboard', { timeout: interval });
  ...
  describe('My Furniture Page', async () => {
    it('The logged-in user should be able to see his/her own furniture (test with 2 furniture)', async () => {
      ...
      await page.goto(host);
      await page.waitForSelector('#profileLink', { timeout: interval });
      await page.click('text=My Publication', { timeout: interval });
      await page.waitForSelector('.card', { timeout: interval });
