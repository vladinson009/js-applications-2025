...
const endpoints = {
  ...
  own: (itemId, userId) =>
    `/data/drones?where=_id%3D%22${itemId}%22%20and%20_ownerId%3D%22${userId}%22&count`,
};
...
describe('E2E tests', function () {
  ...
  describe('Navigation bar [ 10 Points ]', () => {
    ...
    it('Guest user should see correct navigation [ 2.5 Points ]', async function () {
      ...
      expect(await page.isVisible('nav >> text=Sell'), 'Create is visible').to.be
        .false;
  ...
  describe('CRUD [ 50 Points ]', () => {
    describe('Create [ 12.5 Points ]', function () {
      ...
      it('Create makes correct API call [ 2.5 Points ]', async function () {
        ...
        let priceElement = await page.waitForSelector('[name="price"]', {
          timeout: interval,
        });
        let phoneElement = await page.waitForSelector('[name="phone"]', {
          timeout: interval,
        });
        let conditionElement = await page.waitForSelector('[name="condition"]', {
          timeout: interval,
        });
        let weightElement = await page.waitForSelector('[name="weight"]', {
          timeout: interval,
        });
        let descriptionElement = await page.waitForSelector('[name="description"]', {
          timeout: interval,
        });
    ...
    describe('Details [ 10 Points ]', function () {
      it('Details calls the correct API [ 2.5 Points ]', async function () {
        ...
        let moreInfoButton = await page.waitForSelector(
          `.drone:has-text("${data.model}") >> .details-btn`,
          { timeout: interval }
        );
      ...
      it('Details with guest calls shows correct info [ 2.5 Points ]', async function () {
        ...
        await page.click(`.drone:has-text("${data.model}") >> .details-btn`, {
          timeout: interval,
        });

        let imageSrc = await page.getAttribute('#details-img', 'src', {
          timeout: interval,
        });
        ...
        let weight = await page.textContent('.details-weight', {
          timeout: interval,
        });
        let condition = await page.textContent('.details-condition', {
          timeout: interval,
        });
        let phone = await page.textContent('.phone-number', { timeout: interval });
        let about = await page.textContent('.drone-description', {
          timeout: interval,
        });
        ...
        expect(await page.isVisible('.buttons >> text="Delete"')).to.equal(
          false,
          'Delete button was visible for non owner'
        );
        expect(await page.isVisible('.buttons >> text="Edit"')).to.equal(
          false,
    ...
    describe('Edit [ 17.5 Points ]', function () {
      it('Edit calls correct API to populate info [ 2.5 Points ]', async function () {
        ...
        let editButton = await page.waitForSelector('.buttons >> text="Edit"', {
          timeout: interval,
        });
      ...
      it('Edit does NOT work with empty fields [ 2.5 Points ]', async function () {
        ...
        let priceInput = await page.waitForSelector('[name="price"]', {
          timeout: interval,
        });
        let conditionInput = await page.waitForSelector('[name="condition"]', {
          timeout: interval,
        });
        let contactInput = await page.waitForSelector('[name="weight"]', {
          timeout: interval,
        });
        let aboutInput = await page.waitForSelector('[name="description"]', {
          timeout: interval,
        });
    ...
    describe('Delete [ 10 Points ]', function () {
      it('Delete makes correct API call [ 2.5 Points ]', async function () {
        ...
        let deleteButton = await page.waitForSelector('.buttons >> text="Delete"', {
          timeout: interval,
        });
...
async function handleRaw(match, handlers) {
  ...
  function request(method, returns, options) {
    ...
    return {
      onRequest: () =>
        context.waitForRequest(
          (request) => urlPredicate(request) && request.method() === method
        ),
      onResponse: () =>
        context.waitForResponse(
          (response) =>
            urlPredicate(response.request()) &&
