  ...
  catalog: [
    {
      ...
      year: '1870',
      learnMore:
        '1 Lorem ipsum dolor sit amet consectetur adipisicing elit. Sed earum dolores animi impedit, quisquam deleniti distinctio optio vel voluptatibus delectus!',
    ...
    },
    {
      ...
      year: '1858',
      learnMore:
        '2 Lorem ipsum dolor sit amet consectetur adipisicing elit. Sed earum dolores animi impedit, quisquam deleniti distinctio optio vel voluptatibus delectus!',
    ...
    },
    {
      ...
      year: '1945',
      learnMore:
        '3 Lorem ipsum dolor sit amet consectetur adipisicing elit. Sed earum dolores animi impedit, quisquam deleniti distinctio optio vel voluptatibus delectus!',
...
const endpoints = {
  ...
  own: (stampsId, userId) =>
    `/data/stamps?where=_id%3D%22${stampsId}%22%20and%20_ownerId%3D%22${userId}%22&count`,
  like: '/data/likes',
  totalLikes: (stampsId) =>
    `/data/likes?where=stampsId%3D%22${stampsId}%22&distinct=_ownerId&count`,
  userLikes: (stampsId, userId) =>
...
describe('E2E tests', function () {
  ...
  describe('Navigation bar [ 10 Points ]', () => {
    ...
    it('Guest user should see correct navigation [ 2.5 Points ]', async function () {
      ...
      expect(await page.isVisible('nav >> text=Add Stamp'), 'Create is visible').to
        .be.false;
      expect(await page.isVisible('nav >> text=Logout'), 'Logout is visible').to.be
        .false;
      expect(await page.isVisible('nav >> text=Login'), 'Login is not visible').to.be
        .true;
      expect(await page.isVisible('nav >> text=Register'), 'Ragister is not visible')
        .to.be.true;
    });
    ...
    it('Guest user navigation should work [ 2.5 Points ]', async function () {
      ...
      let collectionBtn = await page.waitForSelector('nav >> text=Collection', {
        timeout: interval,
      });
    ...
    it('Logged in user navigation should work [ 2.5 Points ]', async function () {
      ...
      await page.waitForSelector('#collection', { timeout: interval });
      let createBtn = await page.waitForSelector('nav >> text=Add Stamp', {
        timeout: interval,
      });
  ...
  describe('Home Page [ 5 Points ]', function () {
    it('Show Home page text [ 2.5 Points ]', async function () {
      ...
      await page.waitForSelector(
        'text= Explore a world of rare and historic vintage stamps, where collectors connect to exchange knowledge and unique finds. Preserve the art of philately while discovering hidden gems from different eras and regions.',
        { timeout: interval }
      );
      expect(
  ...
  describe('Dashboard Page [ 15 Points ]', function () {
    ...
    it('Check Stamp have correct images [ 2.5 Points ]', async function () {
      ...
      const images = await page.$$eval('.stamp img', (t) => t.map((s) => s.src));
  ...
  describe('CRUD [ 50 Points ]', () => {
    describe('Create [ 12.5 Points ]', function () {
      it('Create does NOT work with empty fields [ 2.5 Points ]', async function () {
        ...
        let addStampBtn = await page.waitForSelector('text=Add Stamp', {
          timeout: interval,
        });
      ...
      it('Create makes correct API call [ 2.5 Points ]', async function () {
        ...
        let nameElement = await page.waitForSelector('[name="name-input"]', {
          timeout: interval,
        });
        let imageElement = await page.waitForSelector('[name="image-url-input"]', {
          timeout: interval,
        });
        let yearElement = await page.waitForSelector('[name="year-input"]', {
          timeout: interval,
        });
    ...
    describe('Details [ 10 Points ]', function () {
      it('Details calls the correct API [ 2.5 Points ]', async function () {
        ...
        let learnMoreButton = await page.waitForSelector(
          `.stamp:has-text("${data.year}") >> .learn-more-btn`,
          { timeout: interval }
        );
      ...
      it('Details with guest calls shows correct info [ 2.5 Points ]', async function () {
        ...
        let nameElement = await page.waitForSelector('#details-name', {
          timeout: interval,
        });
    ...
    describe('Edit [ 17.5 Points ]', function () {
      ...
      it('Edit does NOT work with empty fields [ 2.5 Points ]', async function () {
        ...
        let typeInput = await page.waitForSelector('[name="name-input"]', {
          timeout: interval,
        });
        let imageInput = await page.waitForSelector('[name="image-url-input"]', {
          timeout: interval,
        });
        let descriptionInput = await page.waitForSelector('[name="year-input"]', {
          timeout: interval,
        });
  ...
  describe('BONUS: Likes [ 15 Points ]', async () => {
    ...
    it('Likes for logged in user who has NOT liked calls correct API and shows correct info  [ 2.5 Points ]', async function () {
      ...
      let likesElement = await page.waitForSelector('#likes', { timeout: interval });
      let likeButton = await page.waitForSelector('#action-buttons >> text="Like"', {
        timeout: interval,
      });
      let likes = await likesElement.textContent();
