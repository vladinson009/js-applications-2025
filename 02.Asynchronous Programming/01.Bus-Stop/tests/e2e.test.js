...
describe('E2E tests', function () {
  ...
  describe('List', () => {
    it('Show bus stop name', async () => {
      ...
      get(data);
      await page.waitForSelector('#stopId', { timeout: interval });

      await page.fill('[type="text"]', data._id);
      await page.click('[type="button"]', { timeout: interval });

      await page.waitForSelector('#stopName', { timeout: interval });

      const search = await page.$$eval(`#stopName`, (t) =>
        t.map((s) => s.textContent)
      );
      await page.waitForSelector('#buses>li', { timeout: interval });
